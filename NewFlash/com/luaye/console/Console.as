...
package com.luaye.console {
	...
	public class Console extends Sprite {
		...
		public function Console(pass:String = "", uiset:int = 1) {
			...
			//
			//JEFFREY report("<b>Console v"+VERSION+(VERSION_STAGE?(" "+VERSION_STAGE):"")+", Happy bug fixing!</b>", -2);
			//report("<b>  AWANA-Happy QR Code Scanning! Make sure to reset each new session. </b>", -2);
		...
		private function _onEnterFrame(e:Event):void{
			...
			if(_needToMoveTop && alwaysOnTop && moveTopAttempts>0  && parent){
				...
				if(!quiet){
					//JEFFREY report("Moved console on top (alwaysOnTop enabled), "+moveTopAttempts+" attempts left.",-1);
				}
			}
			if( _isRepeating ){
				_repeated++;
